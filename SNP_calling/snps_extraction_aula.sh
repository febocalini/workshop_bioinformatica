...
# 6.6.  Limpeza dos arquivos .bam com CleanSam
...
# 6.7. Adicionar a informacao do flowcell (read group) nos bams - como as amostras vem do mesmo sequenciamento temos apenas um flowcell e podemos rodar a funcao dessa forma. Caso sejam amostras de varios sequenciamento temos que extrair o flowcell de cada amostra e rodar a funcao de outra forma (ver no final)
...
# 6.8. Agora devemos marcar as duplicatas dos arquivos bam
...
# 6.9. Fix the NM, MD, and UQ tags in a SAM/BAM/CRAM file
...
# 6.10. Indexar fix.bam que acabou de ser criado
...
# A partir desse passo precisamos rodar funcoes que so estao disponiveis apenas no GATK 3.8, por isso, vamos usar o caminho do gatk 3.8 que vou passar para voces
...
# codigo para quando temos varios flowcells diferentes
